...
span <- diff(range(mnist_27$train$x_2))
fit <- loess(as.numeric(as.character(y)) ~ x_2, degree =1, span = span, data = mnist_27$train)
mnist_27$train %>%
  mutate(smooth = fit$fitted, y = as.numeric(as.character(y))) %>%
  ggplot(aes(x_2, y)) +
  geom_point() +
